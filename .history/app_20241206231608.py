...
def process_choice():
    userChoice = request.form.get('options')
    if not userChoice:
        return "No option selected.", 400  # Return a 400 Bad Request response if no option is provided
    return f"Option selected: {userChoice}"  # Return a confirmation message
...
def get_features(image, model, layers=None):
    if layers is None:
        layers = {'0': 'ConvLayer_1', '5': 'ConvLayer_2', '10': 'ConvLayer_3', '19': 'ConvLayer_4', '28': 'ConvLayer_5'}
    features = {}
    x = image
...
def upload_and_style():
    # Get the style choice
    style_choice = request.form.get('options')
    if not style_choice:
        return "No style option selected.", 400  # Handle missing style choice

    # Get the uploaded image
    content_file = request.files.get('content_image')
    if not content_file:
        return "No content image uploaded.", 400  # Handle missing file

    # Save the uploaded image
    content_path = "static/content.jpg"
    content_file.save(content_path)

    # Map style choices to style image paths
    ...
    print(f"Selected style: {style_choice}, Content image saved to {content_path}")
    # pre process
